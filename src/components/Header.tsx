      ...
      <div className="max-w-6xl mx-auto px-3 md:px-6 py-2.5 md:py-3.5 flex items-center justify-between">
        ...
        >
          ...
          <span className="text-base md:text-xl lg:text-2xl font-black tracking-tight text-emerald-500">
            PyLingo
            <span className="text-slate-800 text-[10px] font-bold ml-1.5 align-super bg-slate-100 px-2 py-0.5 rounded-lg border border-slate-200 hidden sm:inline-block">
              v1.0
            </span>
          </span>
        </div>
        ...
        <div className="flex items-center flex-wrap gap-1.5 sm:gap-2 md:gap-4">
          ...
          {/* Vidas (Corações) */}
          <div className="flex items-center space-x-1.5 sm:space-x-2 text-rose-500 font-extrabold bg-rose-50 px-2 sm:px-3.5 py-1 sm:py-1.5 rounded-2xl border-2 border-rose-100 shadow-sm transition-all hover:scale-105">
            <Heart className={`w-4 h-4 sm:w-5 sm:h-5 fill-current ${hearts <= 1 && hearts > 0 ? 'animate-ping' : ''} text-rose-500`} />
            <span className="text-xs sm:text-sm md:text-base">{hearts}<span className="hidden sm:inline"> / 5</span></span>
          </div>
